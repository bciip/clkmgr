...
## MY_EDIT :begin (commented idle_i port, after removing hmac & kmac)
##  // idle hints
##  input [${len(hint_clks)-1}:0] idle_i,
## MY_EDIT :end (commented idle_i port, after removing hmac & kmac)
...
## MY_EDIT : (commented hw2reg, after removing kmac & hmac)
...
% for src in div_srcs:
  ...
  prim_clock_div #(
    ...
    ##MY_EDIT:: .step_down_req_i(step_down_req == lc_ctrl_pkg::On),
...
## MY_EDIT :begin
## original :  .d_i(reg2hw.clk_enables.${k}_en.q),
## modified :  .d_i(reg2hw.clk_enables.q),
...
## MY_EDIT :begin (commented idle hint clk logic, after removing kmac & hmac)
##% for k in hint_clks:
##  logic ${k}_hint;
##  logic ${k}_en;
##% endfor
...
##
#### MY_EDIT :begin (After removing hmac)
#### original :  .d_i(reg2hw.clk_hints.${k}_hint.q),
#### modified :  .d_i(reg2hw.clk_hints.q),
##  prim_flop_2sync #(
##    .Width(1)
##  ) u_${k}_hint_sync (
##    .clk_i(clk_${v["src"]}_i),
##    .rst_ni(rst_${v["src"]}_ni),
##    .d_i(reg2hw.clk_hints.q),
##    .q_o(${k}_hint)
##  );
#### MY_EDIT :end (After removing hmac)
##
##  lc_tx_t ${k}_scanmode;
##  prim_lc_sync #(
##    .NumCopies(1),
...
##% for k,v in hint_clks.items():
#### MY_EDIT :begin (After removing hmac)
####  original : assign hw2reg.clk_hints_status.${k}_val.de = 1'b1;
####             assign hw2reg.clk_hints_status.${k}_val.d = ${k}_en;
####  modified : assign hw2reg.clk_hints_status.de = 1'b1;
####             assign hw2reg.clk_hints_status.d = ${k}_en;
##  
...
##% endfor
##
## MY_EDIT :end (commented idle hint clk logic, after removing kmac & hmac)
